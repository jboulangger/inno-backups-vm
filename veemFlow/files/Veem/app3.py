...
def extract_and_load (min_date, max_date):
    conn = pymssql.connect(host='172.23.8.21',port=1433, user='mquispe', password='C@nvi@cloud2023', database='C4VS03DB')
    cursor = conn.cursor()

    sqlQuery="SELECT "
    sqlQuery=sqlQuery+"'C4VS03DB',"
    ...
    sqlQuery=sqlQuery+" FROM [C4VS03DB].[dbo].[Backup.Model.BackupTaskSessions] b"
    sqlQuery=sqlQuery+" inner join [C4VS03DB].[dbo].[Backup.Model.JobSessions] j on j.orig_session_id=b.session_id"
