...
def extract_and_load (min_date, max_date):
    conn = pymssql.connect(host='172.23.8.20',port=1433, user='mquispe', password='Repo$2@22*', database='master')
    cursor = conn.cursor()

    sqlQuery="SELECT "
    sqlQuery=sqlQuery+"'C4VEEAMSERV',"
    ...
    sqlQuery=sqlQuery+" FROM [VeeamBackup1].[dbo].[Backup.Model.BackupTaskSessions] b"
    sqlQuery=sqlQuery+" inner join [VeeamBackup1].[dbo].[Backup.Model.JobSessions] j on j.orig_session_id=b.session_id"
