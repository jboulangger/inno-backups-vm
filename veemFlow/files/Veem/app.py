...
def extract_and_load (min_date, max_date):
    conn = pymssql.connect(host='172.23.8.21',port=1433, user='mquispe', password='C@nvi@cloud2023', database='SDP-VeeamBackup-DB')
    cursor = conn.cursor()

    sqlQuery="SELECT "
    sqlQuery=sqlQuery+"'SDP-VeeamBackup-DB',"
    ...
    sqlQuery=sqlQuery+" FROM [SDP-VeeamBackup-DB].[dbo].[Backup.Model.BackupTaskSessions] b"
    sqlQuery=sqlQuery+" inner join [SDP-VeeamBackup-DB].[dbo].[Backup.Model.JobSessions] j on j.orig_session_id=b.session_id"
...
def main(days): 
    arr_date = interval_date(date_init=datetime.now())
