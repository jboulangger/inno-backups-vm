...
def extract_and_load (min_date, max_date):
    conn = pymssql.connect(host='172.23.8.120',port=1433, user='mquispe', password='Repo$2@22*', database='master')
    cursor = conn.cursor()

    sqlQuery="SELECT "
    sqlQuery=sqlQuery+"'C1VEEAMSERV',"
    ...
    sqlQuery=sqlQuery+" FROM [DBVEEAM].[dbo].[Backup.Model.BackupTaskSessions] b"
    sqlQuery=sqlQuery+" inner join [DBVEEAM].[dbo].[Backup.Model.JobSessions] j on j.orig_session_id=b.session_id"
...
def main(days): 
    arr_date = interval_date(date_init=datetime.now()-timedelta(days=1), days=days)
